...
1. `app/globals.css`
   ...
   - Verify: adding `class="dark"` to `<html>` in devtools repaints the whole page — background, card, border, and accent — and the design-system `dark:` utilities (card inner shadow, input `bg-input/32`) flip with it.
...
3. `app/layout.tsx`
   ...
   - Verify: hard-reload a dark-preferring browser — no light flash before the dark palette appears.

## Scope

- Inherit: `/`, `/settings`, and all `/<slug>` tool routes; every `components/ui/*` primitive using a `dark:` utility.
- Verify: the paper-grain overlay (`app/globals.css:129-138`, `mix-blend-mode: multiply` at `--grain-opacity`) and the three body radial gradients (`app/globals.css:107-122`), which are authored in light-palette rgba values and are not token-driven — confirm they still read correctly against the dark background.
...
- Product: on `/compress-pdf`, press `d`. The palette flips immediately. Reload — it stays flipped. Press `d` again and reload — it stays light.
...
- System: `grep -rn "prefers-color-scheme" app/ components/` returns only `app/layout.tsx` (`viewport.themeColor`) and the provider's `matchMedia` call — no remaining palette rule keyed on the media query, and no second dark mechanism.
...
- Stop if the inline script cannot run before first paint under the project's Next.js 16 App Router setup without `dangerouslySetInnerHTML` on a `<script>` in `<head>` — reconcile the approach before continuing rather than accepting a theme flash.
- Stop if the change appears to require editing any file under `components/ui/` — those primitives already use the `dark:` variant correctly and inherit the fix.

## Design documentation

- After acceptance and validation: record in `README.md` (or a new `DESIGN.md`) that the dark palette is class-driven — `.dark` on `<html>`, declared via `@custom-variant dark` — and that new dark-specific styling must use the `dark:` variant rather than a `prefers-color-scheme` media query.
