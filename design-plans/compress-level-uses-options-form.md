...
- Interface: check `/compress-pdf` at 375px, 768px, and 1280px. The field should occupy the full width below `sm` and half the grid at `sm` and up, like every other single-option tool. Check its disabled state, open popup, and keyboard focus. The trigger must show the `ring-ring/24` focus ring, not the browser default. Compare it with `/split-pdf`.
...
- Stop if `useOptions` cannot express the level default without re-rendering the panel on every keystroke. That means the field array is recreated per render and must move to module scope.
- Stop if moving to the design-system select changes which value `compressPdf` receives for any of the four levels.
- Stop if the change requires editing `OptionsForm`, `components/ui/select.tsx`, or any file outside the two listed above. That would exceed this plan's scope.
