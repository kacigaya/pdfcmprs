...
- Surface: every tool route `/<slug>` (`app/[slug]/page.tsx`), in the idle state before a run — the output card rendered by `app/components/pdf/ResultCard.tsx`.
...
- Verify: the card's own long-string behavior is unchanged — the replacement is one character shorter than the original and wraps in the same two lines at `lg` width.
...
- Interface: `/compress-pdf` and `/merge-pdf` at 375px, 768px, and 1280px. Confirm the sentence reads correctly in both the stacked and side-by-side branches, and that the idle copy is replaced only in the empty state — the error branch (`status.tone === "error"`) and the result branch are untouched.
...
- Stop if any unit or Playwright spec turns out to assert the old sentence — reconcile the assertion in the same change rather than keeping the copy.
